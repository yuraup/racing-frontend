...
function ResultModal({ myName, round, selectedCard, bots, result, onConfirm }) {
  const resultLabel = result ? RESULT_TEXT[result] : '';
    ...
    <Modal>
      ...
      >
        ...
        <div className="font-basic mr-10 flex flex-col gap-3 text-xs font-bold">
          ...
          {bots.map(bot => (
            <p key={bot.name} className="text-ink">
              {bot.name}: {bot.cardNumber}
            </p>
          ))}
        </div>
